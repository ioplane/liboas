...
**Components:**
- **Parser** (yyjson-based): Streaming JSON parse, YAML via libyaml with JSON conversion
...
**Components:**
- **Schema Compiler**: Walks schema tree, emits flat instruction array — type checks, format validators, constraint checks
- **Path Matcher**: OpenAPI path template matching with parameter extraction (`/users/{id}`)
- **Request/Response Validator**: Header, query parameter, and body validation using compiled schemas
- **Spec Emitter**: Serialize document model back to JSON/YAML (round-trip fidelity)
...
- `oas_schema_t`: JSON Schema node (type, properties, items, allOf/oneOf/anyOf, $ref)
