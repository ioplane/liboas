...
oas_path_matcher_t *oas_path_matcher_create(oas_arena_t *arena,
                                             ...
                                             size_t count)
{
    ...
    oas_path_matcher_t *matcher = oas_arena_alloc(arena, sizeof(*matcher),
                                                     _Alignof(oas_path_matcher_t));
    if (!matcher)
        return nullptr;

    matcher->templates = oas_arena_alloc(arena, count * sizeof(*matcher->templates),
                                         _Alignof(path_template_t));
...
static int split_request_path(const char *path, size_t path_len,
                               const char **seg_starts, size_t *seg_lens,
                               size_t max_segs, size_t *out_count)
{
    *out_count = 0;
...
static bool try_match(const path_template_t *tmpl,
                      const char **seg_starts, const size_t *seg_lens,
                      size_t seg_count)
{
...
int oas_path_match(const oas_path_matcher_t *matcher,
                   ...
                   oas_arena_t *arena)
{
    ...
    int rc = split_request_path(request_path, path_len,
                                seg_starts, seg_lens, MAX_SEGMENTS, &seg_count);
    if (rc < 0) {
    ...
    result->params = oas_arena_alloc(arena, param_count * sizeof(*result->params),
                                     _Alignof(oas_path_param_t));
    if (!result->params)
        return -ENOMEM;
