                             ...
                             oas_compiled_pattern_t **out)
{
    ...
    *out = nullptr;
    ...
    uint8_t *bytecode = lre_compile(&bytecode_len, error_msg, sizeof(error_msg),
                                    pattern, strlen(pattern), LRE_FLAG_UNICODE,
                                    nullptr);
    if (!bytecode)
        return -EINVAL;
...
static bool libregexp_match(oas_regex_backend_t *backend,
                            ...
                            const char *value, size_t len)
{
    ...
    for (int i = 0; i <= input_len; i++) {
        int rc = lre_exec(capture, compiled->bytecode, input, i, input_len,
                          0, nullptr);
        if (rc == 1) {
            free(capture);
            return true;
...
static void libregexp_free_pattern(oas_regex_backend_t *backend,
