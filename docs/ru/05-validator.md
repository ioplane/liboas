...
1. **Точное совпадение** -- например, статус `200` совпадает с ключом ответа `"200"`.
2. **Совпадение по диапазону** -- например, статус `201` совпадает с `"2XX"`.
3. **По умолчанию** -- ответ `"default"` совпадает с любым несовпавшим статусом.
